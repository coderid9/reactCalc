...
function Layout() {
  ...
  const hadleCalc = ()=>{
    try {
      let calcString = calculation.replace('%','*1/100').replace('x','*');
  ...
  return (
    <div id="calc">
      ...
      <div className="calc-buttons">
        ...
        <button className="operator" onClick={()=>setCalculation(prev=>prev+"x")} >x</button>
        <button className="operator" onClick={()=>setCalculation(prev=>prev+"-")} >-</button>
        ...
        <button onClick={()=>setCalculation(prev=>prev.slice(0,[prev.length-1]))} className="cut">
          <span>x</span>
          </button>
